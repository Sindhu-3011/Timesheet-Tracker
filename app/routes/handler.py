"""app/routes/handler.py — HTTP request handler (all GET / POST routes).

parse_multipart and normalize_return_to live here to avoid circular imports.
"""
...
from ..config import (
    DEBUG, HOST, PORT, UPLOAD_DIR, DB_FILE,
    AUTO_LOCK_NTT_ON_OCR,
    DEFAULT_ADMIN_USERNAME, DEFAULT_ADMIN_EMAIL, DEFAULT_ADMIN_PASSWORD,
    IST, UTC, utc_now, utc_now_str, parse_dt_assume_utc, format_dt_ist,
    _safe_name_part, normalize_screen_access, SMTP_HOST, SMTP_PORT,
    SMTP_USER, SMTP_PASS, SMTP_FROM, SMTP_SSL, TESSERACT_DIR,
)
from ..database.connection import get_conn
from ..database.migrations import init_db
from ..database.custom_fields import (
    list_custom_fields, create_custom_field, update_custom_field,
    delete_custom_field, build_custom_fields_form_html,
    validate_and_save_custom_fields_for_user, _parse_options_json,
)
from ..database.users import create_user_db_return_id, ensure_default_admin
from ..database.help_content import (
    ensure_help_table_and_seed, get_help_html, set_help_html,
)
from ..database.user_crud import (
    username_exists, email_exists, create_user_db,
    list_users_db, list_active_users_for_reminders, is_active_user_email,
    get_user_db_by_id, get_user_db_by_email, get_screen_access_by_email,
    get_feature_flags_by_email, get_feature_flags_by_username,
    update_user_db, delete_user_db,
)
from ..database.uploads import (
    _sunday_of_date, _week_start_from_iso_datetime,
    ensure_timesheet_for_upload, add_upload_db, list_uploads_db,
    get_upload_owner, get_upload_owner_by_stored_filename,
    get_upload_module, get_timesheet_by_upload, get_timesheet_values,
    is_timesheet_submitted, mark_timesheet_submitted, upsert_timesheet,
    delete_upload, get_ppm_hours_for_user_week, timesheet_hours_equal,
    list_upload_ids_for_user_week, submit_week_group,
)
from ..ocr.engine import _preprocess_for_ocr
from ..config import _ensure_tesseract_cmd_ready
from ..ocr.ppm import extract_ppm_hours_from_screenshot_total_row
from ..ocr.ntt import extract_ntt_hours_from_screenshot_entered_column
from ..services.email import (
    send_email_smtp, add_email_reminder, list_email_reminders,
    get_email_reminder, update_email_reminder, delete_email_reminder,
    STOP_EMAIL_SCHEDULER,
)
from ..services.export import (
    generate_monthly_export_xlsx, generate_monthly_export_xlsx_filtered,
    build_export_details_table_html, parse_export_filters_from_qs,
    build_export_filter_ui_html,
)
from ..ui.styles import PAGE_STYLE
from ..ui.layout import NAV, build_sidebar_for_role, render_page
from ..ui.components import (
    build_saved_preview_modal, build_timesheet_ui,
    build_timesheet_js, build_capture_ui,
)
from ..auth.sessions import (
    SESSIONS, parse_cookies, make_session, clear_session_from_header,
    get_email_from_request, get_role_by_email,
    get_display_name_by_email,
)
...
if __name__ == "__main__":
    init_db()
    logging.info("Timesheet Tracker v1.1.0 starting...")
    logging.info("URL: http://%s:%s", HOST, PORT)
    logging.info("Debug mode: %s | DB: %s | Uploads: %s", DEBUG, DB_FILE, UPLOAD_DIR)
    try:
